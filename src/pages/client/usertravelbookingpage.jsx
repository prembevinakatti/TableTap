...
const Routing = ({ from, to, setDistance,bikeprice,carprice }) => {
  const map = useMap();
...
const Map = () => {
  ...
  const calculateTotalAmount = () => {
    let amountPerPerson = transportMode === "bike" ? bikeprice : carprice;
  ...
  return (
    <div>
      ...
      >
        ...
        <Routing from={{ lat: 37.7749, lng: -122.4194 }} to={{ lat: 38.5816, lng: -121.4944 }} setDistance={setDistance} />
      </MapContainer>
      ...
      <div className="w-full h-[40vh]  flex items-center justify-center">
        <div>
          ...
          <div className="flex items-center justify-center gap-10">
            ...
            <input type="radio" id="bike" className="radio radio-warning" name="transportMode"  checked={transportMode === "bike"} onChange={() => handleTransportModeChange("bike")} />
              <label htmlFor="bike" className="text-2xl">Bike</label>
            </div>
            <div className="flex items-center m-3 gap-5">
            <input type="radio" id="cab" name="transportMode" className="radio radio-warning" checked={transportMode === "cab"} onChange={() => handleTransportModeChange("cab")}  />
              <label htmlFor="cab" className="text-2xl">Cab</label>
            </div>
          </div>

          <div className="TransportDetails flex flex-col gap-3 w-full h-fit p-5 ">
            <InputBox type="number" placeholder="Number Of Persons" value={numPersons} onChange={handleNumPersonsChange} />
            <InputBox type="text" placeholder="Contact Number" />
            <p>Amount Per Person: {transportMode === "bike" ? 100 : 200}</p>
            <p className="w-full text-center border p-2 bg-primary text-secondary rounded-md text-2xl">Total Amount: {calculateTotalAmount()}</p>
          </div>
        </div>
      </div>
    </div>
